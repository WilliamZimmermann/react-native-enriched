...
/**
 * Decode a numeric entity (`&#231;` / `&#xe9;`) to its character.
 *
 * The editor escapes every non-ASCII character numerically, and so does
 * OneNote: a cell reading "Instituição" arrives as `Institui&#231;&#227;o`.
 * Decoding only the named entities painted that literally in the table.
 */
static NSString *katavDecodeNumericEntities(NSString *s) {
  ...
  if (numericRe == nil || s.length == 0) {
    return s;
  }

  NSMutableString *out = [s mutableCopy];
  NSArray<NSTextCheckingResult *> *matches =
      [numericRe matchesInString:s options:0 range:NSMakeRange(0, s.length)];
  ...
  for (NSTextCheckingResult *m in [matches reverseObjectEnumerator]) {
    NSRange hexRange = [m rangeAtIndex:1];
    NSRange decRange = [m rangeAtIndex:2];
    unsigned long long code = 0;
    if (hexRange.location != NSNotFound) {
      code = strtoull([[s substringWithRange:hexRange] UTF8String], NULL, 16);
    } else if (decRange.location != NSNotFound) {
      code = strtoull([[s substringWithRange:decRange] UTF8String], NULL, 10);
    } else {
      continue;
    }
...
static NSString *katavDecodeEntities(NSString *s) {
  // Numeric first; `&amp;` is undone last below, so an escaped "&amp;#231;"
  // does not turn into a character it never was.
  s = katavDecodeNumericEntities(s);
