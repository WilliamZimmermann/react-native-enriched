...
export const EnrichedTextInput = ({
  ...
  useImperativeHandle(ref, () => ({
    ...
    undo: () => {
      Commands.undo(nullthrows(nativeRef.current));
    },
    redo: () => {
      Commands.redo(nullthrows(nativeRef.current));
    },
    setValue: value => {
    ...
    getSelectionHtml: (start, end) => {
      return new Promise((resolve, reject) => {
        ...
        Commands.requestSelectionHTML(nullthrows(nativeRef.current), requestId, start, end);
      });
    },
    replaceSelectionWithHtml: (start, end, html) => {
      Commands.replaceSelectionWithHtml(nullthrows(nativeRef.current), start, end, html);
    },
    toggleBold: () => {
    ...
    setTextColor: (start, end, color) => {
      Commands.addTextColor(nullthrows(nativeRef.current), start, end, color);
    },
    removeTextColor: (start, end) => {
      Commands.removeTextColor(nullthrows(nativeRef.current), start, end);
    },
    setHighlight: (start, end, color) => {
    ...
    focusTableCell: (tableIndex, row, col) => {
      Commands.focusTableCell(nullthrows(nativeRef.current), tableIndex, row, col);
    },
    ...
    applyAiSuggestion: (start, end, aiId, status, model) => {
      Commands.applyAiSuggestion(nullthrows(nativeRef.current), start, end, aiId, status, model);
    },
    applyAiFlag: (start, end, aiId, status, explanation) => {
      Commands.applyAiFlag(nullthrows(nativeRef.current), start, end, aiId, status, explanation);
    },
    acceptAiMark: aiId => {
      Commands.acceptAiMark(nullthrows(nativeRef.current), aiId);
    },
    rejectAiMark: (aiId, deleteText) => {
