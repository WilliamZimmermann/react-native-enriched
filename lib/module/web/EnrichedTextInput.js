...
export const EnrichedTextInput = ({
  ...
  useImperativeHandle(ref, () => ({
    ...
    setTextColor: (_start, _end, _color) => {},
    removeTextColor: (_start, _end) => {},
    ...
    setSelectedImageCaption: caption => runFocused(editor, c => c.updateAttributes('image', {
      caption: caption || null
    })),
    insertHorizontalRule: () => runFocused(editor, c => c.insertContent('<hr>')),
    // AI track-changes marks are native-only in this wrapper — on web the
    // host app drives them through its own TipTap AiSuggestion extension, so
    // these are parity no-ops (same rationale as setHighlight above).
    applyAiSuggestion: () => {},
    applyAiFlag: () => {},
    acceptAiMark: () => {},
    rejectAiMark: () => {},
